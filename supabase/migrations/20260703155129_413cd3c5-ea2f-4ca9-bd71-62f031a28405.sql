...
BEGIN
  ...
  IF g IS NULL THEN
    RAISE EXCEPTION 'Guía no existe';
  END IF;
  IF g.estado <> 'BORRADOR' THEN
    RAISE EXCEPTION 'Solo se puede emitir una guía en BORRADOR';
  END IF;

  SELECT count(*) INTO v_item_count FROM public.ventas_guia_items WHERE guia_id = p_guia;
  IF COALESCE(v_item_count, 0) = 0 THEN
    RAISE EXCEPTION 'La guía no tiene líneas para emitir';
  END IF;

  FOR it IN SELECT * FROM public.ventas_guia_items WHERE guia_id = p_guia ORDER BY orden NULLS LAST, created_at LOOP
    ...
    v_mov := public.registrar_movimiento(
      p_tipo := 'SALIDA'::public.tipo_mov_t,
      p_lote_id := it.lote_id,
      p_cantidad := it.cantidad_cajas,
      p_ubic_origen := it.ubicacion_id,
      p_ubic_destino := NULL::uuid,
      p_cliente_proveedor := g.cliente_id,
      p_nro_guia := g.codigo,
      p_nro_vale := NULL::text,
      p_motivo := 'Salida por guía de venta',
      p_fecha := COALESCE(g.fecha_emision, CURRENT_DATE),
      p_observaciones := 'Guía ' || g.codigo || COALESCE(' · ' || NULLIF(g.transportista, ''), ''),
      p_nro_warrant := NULL::text,
      p_latas := COALESCE(it.latas, 0),
      p_piso := NULL::integer,
      p_mercado_id := NULL::uuid,
      p_tiene_etiqueta := NULL::boolean,
      p_tercero := NULLIF(g.transportista, ''),
      p_empaque := COALESCE(it.empaque, 48),
      p_donacion := false,
      p_autorizado := NULL::text,
      p_inicia_warrant := NULL::date,
      p_vence_warrant := NULL::date
    );
    ...
    PERFORM set_config('app.bypass_movimientos_block', 'false', true);
...
REVOKE ALL ON FUNCTION public.ventas_emitir_guia(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.ventas_emitir_guia(uuid) TO authenticated, service_role;