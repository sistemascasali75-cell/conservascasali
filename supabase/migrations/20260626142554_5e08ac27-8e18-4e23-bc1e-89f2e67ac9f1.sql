...
ALTER TABLE public.movimientos
  ADD COLUMN IF NOT EXISTS empaque integer NOT NULL DEFAULT 48,
  ADD COLUMN IF NOT EXISTS donacion boolean NOT NULL DEFAULT false,
...
CREATE OR REPLACE FUNCTION public.registrar_movimiento(
  ...
  p_donacion boolean DEFAULT false,
  p_autorizado text DEFAULT NULL::text
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
REVOKE EXECUTE ON FUNCTION public.registrar_movimiento(tipo_mov_t, uuid, numeric, uuid, uuid, uuid, text, text, text, date, text, text, integer, integer, uuid, boolean, text, integer, boolean, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.registrar_movimiento(tipo_mov_t, uuid, numeric, uuid, uuid, uuid, text, text, text, date, text, text, integer, integer, uuid, boolean, text, integer, boolean, text) TO authenticated, service_role;
